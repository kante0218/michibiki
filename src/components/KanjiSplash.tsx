...
// KanjiVG stroke paths for 導 (15 strokes)
// Source: KanjiVG project (CC BY-SA 3.0) by Ulrich Apel
const STROKES = [
  "M51.56,12.37c1.82,1.37,4.71,5.63,5.17,7.75",
  ...
  "M42.62,83.41c1.93,1.62,4.97,6.67,5.45,9.19",
];

// Approximate length for each stroke (for dasharray animation)
const STROKE_LENGTHS = [
  30, 25, 120, 20, 55, 80, 50, 50, 50, 30, 100, 130, 140, 60, 30,
];

const STROKE_DELAY = 0.18; // seconds between each stroke start
const STROKE_DURATION = 0.4; // seconds per stroke draw

export default function KanjiSplash({ onComplete }: { onComplete: () => void }) {
  const [phase, setPhase] = useState<"drawing" | "pause" | "fadeout" | "done">("drawing");

  const totalDrawTime = STROKES.length * STROKE_DELAY + STROKE_DURATION;

  useEffect(() => {
    // After all strokes are drawn, pause briefly
    ...
    const fadeTimer = setTimeout(() => {
      setPhase("fadeout");
    }, (totalDrawTime + 0.5) * 1000);

    // Then complete
    const doneTimer = setTimeout(() => {
      setPhase("done");
      onComplete();
    }, (totalDrawTime + 1.3) * 1000);
  ...
  }, [onComplete, totalDrawTime]);

  if (phase === "done") return null;

  return (
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 transition-opacity duration-700 ${
    ...
    >
      <div className="flex flex-col items-center">
        ...
        >
          {STROKES.map((d, i) => {
            const len = STROKE_LENGTHS[i];
            const delay = i * STROKE_DELAY;
            return (
              <path
                key={i}
                d={d}
                ...
                style={{
                  animation: `drawStroke ${STROKE_DURATION}s ease-out ${delay}s forwards`,
                }}
              />
            );
          })}
        </svg>
        {/* Brand name fades in after kanji is drawn */}
        <div
          className="mt-6 opacity-0"
          style={{
            animation: `fadeIn 0.6s ease-out ${totalDrawTime - 0.2}s forwards`,
          }}
        >
          <span className="text-2xl font-light text-indigo-900/70 tracking-[0.3em]">
            みちびき
          </span>
        </div>
      </div>
